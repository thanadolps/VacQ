...
exports.getAppointments = async (req, res, next) => {
  let query;
  // General user can only see their own appointments
...
exports.addAppointment = async (req, res, next) => {
  ...
  try {
    ...
    // Check for existed apponintment
    const existedAppointment = await Appointment.find({
      user: req.user.id,
    });
    // If the user is not admin, they can only create three appointments
    if (existedAppointment.length >= 3 && req.user.role !== "admin") {
      return res.status(400).json({
        success: false,
        message: `The user with ID ${req.user.id} has already created three appointments`,
      });
    }
